...
### Examples
demoToy is currently the only example.  It is kind of complex, uses a web server, web sockets, and neopixel animations, so it is not really a great entry level example.  That said, it does some pretty cools stuff. [here is a video of the demo](https://www.youtube.com/watch?v=hqjOY8YHdlM&feature=youtu.be)
...
Initialize the mesh network.  This routine does the following:
...
###v oid easyMesh::setNewConnectionCallback( &newConnectionCallback )
